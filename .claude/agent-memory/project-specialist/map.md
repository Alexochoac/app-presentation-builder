...
│   ├── [presId]/                   ← presId is now number-only format (e.g. 00000001)
...
## presentations.json Structure & Presentation ID Format (UPDATED 2026-04-26)
...
**Presentation ID Format Changed (2026-04-26):**
- **Function `makePresId()` in `builder/server.js`** now returns zero-padded 8-digit numbers (`00000001`, `00000002`, etc.)
- **Old format:** slug-based IDs like `softsolution-00000001` (included `customerName` parameter)
- **New format:** number-only IDs like `00000001` (no customer name in ID; `customerName` parameter removed)
- **Migration:** Existing presentations in `presentations.json` may still use old slug format; **only new presentations** get number-only IDs
- **Folder naming:** `finished-presentations/` folders are now named by number only (e.g., `finished-presentations/00000002/index.html`)
- **Backwards compatibility:** Old slug-style folders remain on disk for existing presentations (e.g., `finished-presentations/softsolution-00000004/`)
